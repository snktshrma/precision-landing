...
def main():
    ...
    # t1 = threading.Thread(target=mode.recvLocal)
    ...
    mode.setPos(target_pos[0], target_pos[1],0)
    ...
    while True:
        ...
        if not pos:
            ...
            if abs(pos[0]) >= 0.2 and abs(pos[1]) >= 0.2:      ##### confirm if loop is necessary or a single iteration is accurate enough for getting x and y errors under 0
                mode.setPos(pos[0],pos[1],0)
                time.sleep(2)
            # else:
